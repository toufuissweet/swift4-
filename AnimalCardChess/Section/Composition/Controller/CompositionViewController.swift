...
class CompositionViewController : BaseViewController{
    ...
    @objc func didBackButton(){
        let alertView = UIAlertController(title: "确定返回", message: "返回后游戏就结束了", preferredStyle: .alert)
        let actionSure = UIAlertAction(title: "确认", style: .default){(action)in
            self.back()
        }
