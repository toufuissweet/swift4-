...
class AIManager: NSObject {
    ...
    func think(alpha: Int, beta: Int, depth: Int, move: Move!)->Move{
        ...
        if depth == 0{
            return Move(value: self.evalute(), step: move.step)
        }
        //搜索所有走法
        ...
        for step in moveSteps{
            ...
            //走完这步获胜则直接返回，不需要再继续思考对方的走棋过程
            if self.curManager.judgeGameOver(){
                //撤销这步走棋（只思考过程，所以走棋需要撤销）
            ...
            self.curManager.switchPlayer()
            //走棋后记录当前的局面值
            let nextMove = Move(value: self.evalute(), step: step)
            //思考对手走棋（思考过程递归调用）
            let otherMove = self.think(alpha: -beta, beta: -myAlpha, depth: depth-1, move: nextMove)//取反是为了获取自己的局面值
            let otherVal = -otherMove.value
            //撤销这步走棋（思考过对方的走棋情况再撤销）
            self.curManager.regret()
        ...
        //没有最佳根节点的走法，说明怎么走都得不到有利的局面，那么就随机返回一个走法（增加随机性）
